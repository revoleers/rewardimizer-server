<?php
/**
 * This code comes from http://jasonpriem.com/human-name-parse/
 * 
 * Works with a Name object to parse out the parts of a name.
 *
 * Example usage:
 *      $parser = new Revoleers_Util_Search_Parser("John Q. Smith");
 *      echo  $parser->getLast() . ", " . $parser->getFirst();
 *      //returns "Smith, John"
 *
 *
 */
class Revoleers_Util_Search_Parser {
	 private $name;
     private $leadingInit;
     private $first;
     private $nicknames;
      ...
      */
      public function setName($name = NULL){
          if ($name) {
              ...
              if (is_object($name) && get_class($name) == "Revoleers_Util_Search_Name") { // this is mostly for testing
                  $this->name = $name;
              }
              else {
                  $this->name = new Revoleers_Util_Search_Name($name);
              }
